...
const AUDIENCE_COLORS = {
  ALL: {
    primary:
      "bg-gradient-to-r from-[#FF7E4B] via-[#FF518C] text-white to-[#66319B] min-w-[210px] after:absolute after:w-[98%] after:top-1/2 after:left-1/2 after:!bg-[#0e1f35] after:h-[93%] after:-z-10 relative z-20 after:-translate-x-1/2 after:-translate-y-1/2 after:rounded-md after:opacity-0 after:duration-300 rounded-md",
    secondary:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, audience, level, size, asChild = false, parentClassName, disabled, icon, iconPosition = "left", children, ...props }, ref) => {
    ...
    const content = (
      <span className={cn("flex items-center gap-2", iconPosition === "right" && "flex-row-reverse")}>
        {icon}
        {children}
      </span>
    );
