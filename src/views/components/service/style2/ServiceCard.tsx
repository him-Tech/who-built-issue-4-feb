  ...
  return (
    <div className="rounded-2xl h-full service-card group 3xl:rounded-[35px] sm:min-w-[470px] w-full sm:max-w-[668px] bg-[rgba(20,35,58,0.79)] backdrop-blur-[35px] !px-4 !pt-4 !pb-7">
      <div className="flex items-center gap-2.5 md:gap-4 leading-[100%] bg-[#0A1930] !p-4 lg:!p-6 3xl:!p-9 rounded-xl xl:rounded-[25px] font-michroma text-white">
        ...
        <h1 className="text-lg sm:text-xl md:text-2xl lg:text-[28px] min-[1880px]:text-[35px]">{props.title}</h1>
      </div>
        ...
        {props.buttonText && (
          <ExternalLink href={laurianeCalendly} underline={false}>
            <button className="relative w-full min-w-[210px] rounded-xl lg:rounded-[18px] h-14 text-lg 3xl:h-[61px] 3xl:text-2xl before:absolute before:inset-0 before:rounded-[18px] before:transition-opacity before:duration-300 before:ease-linear hover:before:opacity-0 bg-gradient-custom">
              ...
              <span className="relative z-20 text-white">{props.buttonText}</span>
            </button>
          </ExternalLink>
        )}
      </div>
